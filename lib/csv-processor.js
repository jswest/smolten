...
class CSVProcessor {
  ...
  async generateOntology(csvPath, ontologyPath, options = {}) {
    ...
    const args = [
      join(AGENTS_DIR, "ontology_generator.py"),
      csvPath,
      ontologyPath,
      "--model-provider", this.config.modelProvider,
  ...
  runPythonAgent(args, operationName) {
    return new Promise((resolve, reject) => {
      logger.info(`Starting ${operationName}`, { 
        ...
        args: args.map(arg => arg.includes(PROJECT_ROOT) ? `.../${arg.split('/').pop()}` : arg)
      });
      ...
      childProcess.on("error", (error) => {
        ...
        reject(new Error(`${operationName} process error: ${error.message}`));
      });
    });
  }
}
