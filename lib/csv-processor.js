...
class CSVProcessor {
  ...
  async processCSV(inputPath, outputPath, options = {}) {
    ...
    if (existsSync(outputPath)) {
      ...
      if (!shouldOverwrite) {
        cliOutput.info("Wise choice! smolten keeps your data safe.", "🛡️");
      ...
      cliOutput.info("Alright, let's melt that old file into something better!", "🌶️");
    }
    ...
    if (!skipOntology && !ontologyPath) {
      ...
      if (shouldReview) {
        cliOutput.info(`Your ontology gems are cooling at: ${ontologyPath}`, "🗂️");
        cliOutput.info("Take a peek, then we'll continue the magic...", "🔍");
        
        cliOutput.prompt();
        await askQuestion("Ready to melt some data? Press Enter to start tagging...");
      }
